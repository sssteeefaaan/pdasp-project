...
type Automobile struct {
	...
	CurrentMalfunctions map[string]Malfunction `json:"currentMalfunction"`
}
...
func (a Automobile) GetId() string {
	return fmt.Sprintf("%s:%s", TypeAutomobile, a.ID)
}

func (p Person) GetId() string {
	return fmt.Sprintf("%s:%s", TypePerson, p.ID)
}

func (m Malfunction) GetId() string {
	return fmt.Sprintf("%s:%s", TypeMalfunction, m.ID)
}
...
func (s *SmartContract) ItemExists(ctx contractapi.TransactionContextInterface, id string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("Failed to read item with id (%s) from world state: %v", id, err)
	}
...
func (s *SmartContract) CreateAutomobile(
	...
	automobileId := CreateId(TypeAutomobile, id)
	exists, err := s.ItemExists(ctx, automobileId)
	if err != nil {
		return nil, err
	}
	if exists {
	...
	personId := CreateId(TypePerson, ownerId)
	exists, err = s.ItemExists(ctx, personId)
	if err != nil {
		return nil, err
	}
	if !exists {
...
func (s *SmartContract) CreatePerson(
	...
	personId := CreateId(TypePerson, id)
	exists, err := s.ItemExists(ctx, personId)
	if err != nil {
		return nil, err
	}
	...
	err = ctx.GetStub().PutState(person.GetId(), personJSON)
	if err != nil {
		return nil, fmt.Errorf("Failed to put person(%s) to world state: %v", personId, err)
	}
...
func (s *SmartContract) GetAllPeople(ctx contractapi.TransactionContextInterface) ([]*Person, error) {
	peopleIterator, err := ctx.GetStub().GetStateByRange(fmt.Sprintf("%s:0", TypePerson), fmt.Sprintf("%s:%d", TypePerson, TotalQueryLimit))
	if err != nil {
		return nil, err
	}
...
func (s *SmartContract) GetAllAutomobiles(ctx contractapi.TransactionContextInterface) ([]*Automobile, error) {
	automobilesIterator, err := ctx.GetStub().GetStateByRange(fmt.Sprintf("%s:0", TypeAutomobile), fmt.Sprintf("%s:%d", TypeAutomobile, TotalQueryLimit))
	if err != nil {
		return nil, err
	}
