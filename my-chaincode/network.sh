...
export PEER_PER_ORGANIZATION_NUMBER=4
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    generateCryptogenConfiguration
    ...
    do
        ...
        set -x
        cryptogen generate --config=${CRYPTOGEN_PATH}/crypto-config-org$i.yaml --output="${ORGANIZATIONS_PATH}"
        res=$?
        { set +x; } 2>/dev/null
        if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=${CRYPTOGEN_PATH}/crypto-config-orderer.yaml --output="${ORGANIZATIONS_PATH}"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function generateCADockerConfiguration(){
  echo "version: '2.1'
...
function generateCryptogenConfiguration(){
  ...
  - Name: Org$i
    ...
    EnableNodeOUs: true
    Template:
      Count: $PEER_PER_ORGANIZATION_NUMBER
      Start: 0
      SANS:
        - localhost
    Users:
      Count: 1" >${CRYPTOGEN_PATH}/crypto-config-org$i.yaml
  done
}
